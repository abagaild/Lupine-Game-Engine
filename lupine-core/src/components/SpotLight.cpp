...
void SpotLight::InitializeDebugMesh() {
    if (m_debug_mesh_initialized) {
        ...
        if (m_debug_cone_vao != 0) {
            glDeleteVertexArrays(1, &m_debug_cone_vao);
            glDeleteBuffers(1, &m_debug_cone_vbo);
            glDeleteBuffers(1, &m_debug_cone_ebo);
        }
    }
...
void SpotLight::RenderDebugVisualization(Node3D* node3d) {
    if (!node3d || !m_debug_mesh_initialized || m_debug_cone_vao == 0) {
        return;
    }
...
void SpotLight::GenerateConeMesh(float range, float angle) {
    ...
    glGenVertexArrays(1, &m_debug_cone_vao);
    glGenBuffers(1, &m_debug_cone_vbo);
    glGenBuffers(1, &m_debug_cone_ebo);

    // Bind VAO
    glBindVertexArray(m_debug_cone_vao);

    // Bind and setup VBO
    glBindBuffer(GL_ARRAY_BUFFER, m_debug_cone_vbo);
    glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(float), vertices.data(), GL_STATIC_DRAW);

    // Bind and setup EBO
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_debug_cone_ebo);
