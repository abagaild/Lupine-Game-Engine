...
void NinePatchPanel::RenderNinePatchNode2D(Node2D* node2d) {
    ...
    Renderer::RenderQuad(transform, m_modulate_color, Renderer::GetWhiteTexture());
}
...
void NinePatchPanel::RenderPatch(const glm::vec2& position, const glm::vec2& size, 
    ...
    // TODO: Convert m_texture_id to GraphicsTexture - for now use white texture
    Renderer::RenderQuad(transform, m_modulate_color, Renderer::GetWhiteTexture(), texture_region);
}
