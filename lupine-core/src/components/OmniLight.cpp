...
OmniLight::OmniLight()
    ...
    , m_debug_sphere_vao(0)
    , m_debug_sphere_vbo(0)
    , m_debug_sphere_ebo(0)
...
void OmniLight::InitializeDebugMesh() {
    if (m_debug_mesh_initialized) {
        ...
        if (m_debug_sphere_vao != 0) {
            glDeleteVertexArrays(1, &m_debug_sphere_vao);
            glDeleteBuffers(1, &m_debug_sphere_vbo);
            glDeleteBuffers(1, &m_debug_sphere_ebo);
        }
    }
...
void OmniLight::RenderDebugVisualization(Node3D* node3d) {
    if (!node3d || !m_debug_mesh_initialized || m_debug_sphere_vao == 0) {
        return;
    }
...
void OmniLight::GenerateSphereMesh(float radius, int segments) {
    ...
    glGenVertexArrays(1, &m_debug_sphere_vao);
    glGenBuffers(1, &m_debug_sphere_vbo);
    glGenBuffers(1, &m_debug_sphere_ebo);

    // Bind VAO
    glBindVertexArray(m_debug_sphere_vao);

    // Bind and setup VBO
    glBindBuffer(GL_ARRAY_BUFFER, m_debug_sphere_vbo);
    glBufferData(GL_ARRAY_BUFFER, vertices.size() * sizeof(float), vertices.data(), GL_STATIC_DRAW);

    // Bind and setup EBO
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_debug_sphere_ebo);
