...
bool OpenGLDevice::InitializeOpenGL() {
    // Check if GLAD is already initialized by checking if a basic function is available
    if (glGetString == nullptr) {
        // GLAD not initialized yet, try to initialize it
        if (!gladLoadGL()) {
    ...
    // Verify OpenGL context is current and working
    const GLubyte* version = glGetString(GL_VERSION);
    if (!version) {
        std::cerr << "Failed to get OpenGL version - no current context?" << std::endl;
        return false;
    }
    ...
    std::cout << "OpenGL context verified: " << version << std::endl;
    return true;
}
