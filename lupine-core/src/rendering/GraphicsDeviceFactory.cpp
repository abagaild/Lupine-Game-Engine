...
std::shared_ptr<GraphicsDevice> GraphicsDeviceFactory::CreateDevice(const DeviceParams& params) {
    ...
    if (backend == GraphicsBackend::None) {
        std::cerr << "No suitable graphics backend found. Available backends: " << available.size() << std::endl;
        std::cerr << "Make sure graphics backend compilation defines are set correctly." << std::endl;
        return nullptr;
    }
...
bool GraphicsDeviceFactory::IsBackendAvailable(GraphicsBackend backend) {
    switch (backend) {
        case GraphicsBackend::OpenGL:
            // OpenGL is available on desktop platforms if compiled in
#if defined(LUPINE_OPENGL_BACKEND) && !defined(__EMSCRIPTEN__)
            return true;
#else
            return false;
#endif
        ...
        case GraphicsBackend::WebGL:
            // WebGL is available on web platforms if compiled in
#if defined(LUPINE_WEBGL_BACKEND) && defined(__EMSCRIPTEN__)
            return true;
#else
            return false;
#endif
