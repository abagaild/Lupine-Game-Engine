...
bool Engine::InitializeForEditor() {
    if (m_initialized) {
        return true;
    }

    std::cout << "Initializing Lupine Engine for editor mode..." << std::endl;
    ...
    // ViewportManager is a static utility class, no initialization needed

    // Don't initialize renderer in editor mode - the editor will do it after creating the window
    // The renderer requires an OpenGL context which is created by the editor's UI system
    ...
    // Don't create a default camera in editor mode
    // The editor will manage its own cameras

    m_initialized = true;
    std::cout << "Engine initialized for editor mode successfully!" << std::endl;
    return true;
}
