...
DirectionalLight::DirectionalLight()
    ...
    , m_debug_arrow_vao(nullptr)
    , m_debug_arrow_vbo(nullptr)
    , m_debug_arrow_ebo(nullptr)
...
void DirectionalLight::InitializeDebugMesh() {
    if (m_debug_mesh_initialized) {
        ...
        m_debug_arrow_vao.reset();
        m_debug_arrow_vbo.reset();
        m_debug_arrow_ebo.reset();
    }
...
void DirectionalLight::RenderDebugVisualization(Node3D* node3d) {
    if (!node3d || !m_debug_mesh_initialized || !m_debug_arrow_vao) {
        return;
    }
...
void DirectionalLight::GenerateArrowMesh(float length) {
    ...
    if (!graphics_device) {
        std::cerr << "DirectionalLight: No graphics device available for debug mesh creation" << std::endl;
        return;
    }

    // Create vertex buffer
    m_debug_arrow_vbo = graphics_device->CreateBuffer(
        BufferType::Vertex,
        BufferUsage::Static,
    ...
    // Create index buffer
    m_debug_arrow_ebo = graphics_device->CreateBuffer(
        BufferType::Index,
        BufferUsage::Static,
    ...
    // Create vertex array
    m_debug_arrow_vao = graphics_device->CreateVertexArray();

    if (!m_debug_arrow_vao || !m_debug_arrow_vbo || !m_debug_arrow_ebo) {
        std::cerr << "DirectionalLight: Failed to create debug mesh graphics objects" << std::endl;
        return;
    }
    ...
    m_debug_arrow_vao->SetVertexBuffer(m_debug_arrow_vbo, layout);
    m_debug_arrow_vao->SetIndexBuffer(m_debug_arrow_ebo);
}
