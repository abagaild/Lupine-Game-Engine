...
void TextureRectangle::LoadTexture() {
    ...
    Texture texture = ResourceManager::LoadTexture(m_texture_path, true);
    if (texture.IsValid()) {
        m_texture = texture.graphics_texture;
        m_texture_loaded = true;
        m_texture_size = glm::vec2(static_cast<float>(texture.width), static_cast<float>(texture.height));
        std::cout << "TextureRectangle: Successfully loaded texture: " << m_texture_path
                  << " (" << texture.width << "x" << texture.height << ")" << std::endl;
    } else {
        std::cerr << "TextureRectangle: Failed to load texture: " << m_texture_path << std::endl;
        m_texture = nullptr;
        m_texture_loaded = false;
        m_texture_size = glm::vec2(100.0f, 100.0f); // Default size for layout purposes
    }
}

void TextureRectangle::RenderTexture(Control* control) {
    if (!control || !m_texture_loaded || !m_texture || !m_texture->IsValid()) return;
    ...
    // Render texture quad using the graphics texture
    Renderer::RenderQuad(transform, m_modulate_color, m_texture, m_texture_region, m_flip_h, m_flip_v);
}

void TextureRectangle::RenderTextureNode2D(Node2D* node2d) {
    if (!node2d || !m_texture_loaded || !m_texture || !m_texture->IsValid()) return;
