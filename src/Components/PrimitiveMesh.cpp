...
void PrimitiveMesh::OnUpdate(float delta_time) {
    ...
    if (m_mesh_generated && m_vertex_array) {
        ...
        if (hasValidOwner) {
            ...
            if (m_has_textures && !m_textures.empty()) {
                // Use the first texture (diffuse map)
                std::shared_ptr<GraphicsTexture> texture = m_textures[0];
                Renderer::RenderMesh(temp_mesh, transform, m_color, texture, m_receives_shadows);
            } else {
...
void PrimitiveMesh::LoadExternalMesh() {
    ...
    for (const auto& texture : mesh.material.diffuse_maps) {
        if (texture.texture && texture.texture->IsValid()) {
...
void PrimitiveMesh::CreateMeshBuffers(const std::vector<float>& vertices, const std::vector<unsigned int>& indices) {
    ...
    if (!graphics_device) {
        std::cerr << "PrimitiveMesh: No graphics device available" << std::endl;
        return;
    }

    try {
        // Clear existing buffers (automatic cleanup via shared_ptr)
        m_vertex_array = nullptr;
        m_vertex_buffer = nullptr;
        m_index_buffer = nullptr;
        ...
        if (!m_vertex_array) {
            std::cerr << "PrimitiveMesh: Failed to create vertex array" << std::endl;
            return;
        }
        ...
        if (!m_vertex_buffer) {
            std::cerr << "PrimitiveMesh: Failed to create vertex buffer" << std::endl;
            return;
        }
        ...
        if (!m_index_buffer) {
            std::cerr << "PrimitiveMesh: Failed to create index buffer" << std::endl;
            return;
        }
        ...
        std::cerr << "PrimitiveMesh: Exception during mesh buffer creation: " << e.what() << std::endl;
        // Cleanup on error
        m_vertex_array = nullptr;
        m_vertex_buffer = nullptr;
        m_index_buffer = nullptr;
...
void PrimitiveMesh::RenderForShadows() {
    if (!m_mesh_generated || !m_vertex_array || !m_vertex_array->IsValid() || !m_casts_shadows) {
        return;
    }
    ...
    // Bind vertex array and render for shadow mapping using unified API
    m_vertex_array->Bind();

    if (m_index_count > 0) {
        // Render with indices
        graphics_device->DrawIndexed(PrimitiveType::Triangles, m_index_count, 0);
    } else if (m_vertex_count > 0) {
        // Render without indices
        graphics_device->Draw(PrimitiveType::Triangles, m_vertex_count, 0);
    }
