        ...
        if (hasValidOwner) {
            try {
                ...
                for (const auto& mesh : meshes) {
                    ...
                    // Debug: Log mesh rendering attempt
                    static int debug_counter = 0;
                    if (++debug_counter % 120 == 0) { // Log every 120 frames
                        std::cout << "StaticMesh::OnUpdate: Rendering mesh with "
                                  << mesh.vertices.size() << " vertices, "
                                  << mesh.indices.size() << " indices, VAO: "
                                  << (mesh.VAO ? "valid" : "null") << std::endl;
                    }

                    // Check if mesh has textures
                    if (!mesh.material.diffuse_maps.empty() && mesh.material.diffuse_maps[0].id() != 0) {
                        // Render with texture
                        Renderer::RenderMesh(mesh, transform, m_color, mesh.material.diffuse_maps[0].texture);
                    } else {
                        // Render without texture
                        Renderer::RenderMesh(mesh, transform, m_color, Renderer::GetWhiteTexture());
                    }
                }
    ...
    for (const auto& mesh : meshes) {
        ...
        if (mesh.VAO) {
            ...
            if (!mesh.indices.empty()) {
                // Render with indices
                graphics_device->DrawIndexed(PrimitiveType::Triangles, static_cast<int>(mesh.indices.size()), 0);
            } else if (!mesh.vertices.empty()) {
                // Render without indices
                graphics_device->Draw(PrimitiveType::Triangles, static_cast<int>(mesh.vertices.size()), 0);
            }
