...
Skybox3D::Skybox3D()
    ...
    , m_skybox_VAO(nullptr)
    , m_skybox_VBO(nullptr)
    , m_texture(nullptr)
...
void Skybox3D::Render() {
    // Debug: Check if skybox is being called
    static bool debug_logged = false;
    if (!debug_logged) {
        std::cout << "Skybox3D: Render() called with mode " << static_cast<int>(m_skybox_mode) << std::endl;
        std::cout << "Skybox3D: Background color: " << m_background_color.r << ", " << m_background_color.g << ", " << m_background_color.b << ", " << m_background_color.a << std::endl;
        debug_logged = true;
    }
...
void Skybox3D::InitializeMesh() {
    ...
    if (!graphics_device) {
        std::cerr << "Skybox3D: No graphics device available" << std::endl;
        return;
    }

    try {
        // Create vertex array
        m_skybox_VAO = graphics_device->CreateVertexArray();
        if (!m_skybox_VAO) {
            std::cerr << "Skybox3D: Failed to create vertex array" << std::endl;
            return;
        }

        // Create vertex buffer
        m_skybox_VBO = graphics_device->CreateBuffer(
            BufferType::Vertex,
            BufferUsage::Static,
        ...
        if (!m_skybox_VBO) {
            std::cerr << "Skybox3D: Failed to create vertex buffer" << std::endl;
            return;
        }
        ...
        // Set up vertex array with buffer and layout
        m_skybox_VAO->Bind();
        m_skybox_VAO->SetVertexBuffer(m_skybox_VBO, layout);
        m_skybox_VAO->Unbind();

        m_mesh_initialized = true;

    } catch (const std::exception& e) {
        std::cerr << "Skybox3D: Exception during mesh initialization: " << e.what() << std::endl;
        // Cleanup on error
        m_skybox_VAO = nullptr;
        m_skybox_VBO = nullptr;
    }
...
void Skybox3D::SetFogUniforms() {
    ...
    // Set fog parameters - convert vec4 to vec3 where needed
    s_skybox_shader->SetUniform("fogEnabled", m_fog_enabled);
    s_skybox_shader->SetUniform("fogColor", glm::vec3(m_fog_color));
    s_skybox_shader->SetUniform("fogDensity", m_fog_density);
    s_skybox_shader->SetUniform("fogStart", m_fog_start);
    s_skybox_shader->SetUniform("fogEnd", m_fog_end);
    s_skybox_shader->SetUniform("fogHeightFalloff", m_fog_height_falloff);
    ...
    s_skybox_shader->SetUniform("cameraPosition", camera_pos);
}
...
void Skybox3D::InitializeShaders() {
    ...
    // ShaderManager will check if it's initialized in LoadShader method
    ...
    s_skybox_shader = ShaderManager::LoadShader("skybox", skybox_vertex, skybox_fragment);

    if (!s_skybox_shader || !s_skybox_shader->IsValid()) {
        std::cerr << "Skybox3D: Failed to load skybox shader" << std::endl;
        if (s_skybox_shader) {
            std::cerr << "Skybox3D: Shader compile log: " << s_skybox_shader->GetCompileLog() << std::endl;
        }
        return;
    }

    std::cout << "Skybox3D: Successfully loaded skybox shader" << std::endl;
    s_shaders_initialized = true;
}

void Skybox3D::RenderSolidColor() {
    if (!s_skybox_shader || !m_mesh_initialized) {
        std::cerr << "Skybox3D: Shader or mesh not initialized for solid color rendering" << std::endl;
        return;
    }

    // Get graphics device for state management
    auto graphics_device = GraphicsDeviceFactory::GetDevice();
    if (!graphics_device) {
        std::cerr << "Skybox3D: Graphics device not available" << std::endl;
        return;
    }

    // For skybox rendering: disable depth writing but keep depth testing
    // This ensures skybox renders behind everything but doesn't write to depth buffer
    graphics_device->SetDepthTest(true);
    graphics_device->SetDepthWrite(false);
    graphics_device->SetDepthFunc(DepthFunc::LessEqual);

    s_skybox_shader->Bind();

    // Set uniforms
    s_skybox_shader->SetUniform("skyboxMode", 0);
    s_skybox_shader->SetUniform("backgroundColor", m_background_color);

    // Set sun parameters (even for solid color mode) - convert vec4 to vec3 where needed
    s_skybox_shader->SetUniform("sunEnabled", m_sun_enabled);
    s_skybox_shader->SetUniform("sunDirection", GetSunDirection());
    s_skybox_shader->SetUniform("sunColor", glm::vec3(m_sun_color));
    s_skybox_shader->SetUniform("sunIntensity", m_sun_intensity);
    s_skybox_shader->SetUniform("shadowColor", glm::vec3(m_shadow_color));
    ...
    s_skybox_shader->SetUniform("view", view);
    s_skybox_shader->SetUniform("projection", Renderer::GetProjectionMatrix());

    // Render skybox cube using unified graphics API
    if (m_skybox_VAO) {
        m_skybox_VAO->Bind();
        graphics_device->Draw(PrimitiveType::Triangles, 36, 0);
        m_skybox_VAO->Unbind();
    }

    // Re-enable depth writing and reset depth function for subsequent rendering
    graphics_device->SetDepthWrite(true);
    graphics_device->SetDepthFunc(DepthFunc::Less);
    graphics_device->SetDepthTest(true);
}
...
void Skybox3D::RenderPanoramicTexture() {
    ...
    if (m_skybox_mode == SkyboxMode::PanoramicImage) {
        s_skybox_shader->SetUniform("skyboxMode", 1);
        if (m_texture_loaded && m_texture && m_texture->IsValid()) {
            // Bind texture using graphics abstraction
            m_texture->Bind(0);
            s_skybox_shader->SetUniform("panoramicTexture", 0);
        }
    } else if (m_skybox_mode == SkyboxMode::Cubemap) {
        s_skybox_shader->SetUniform("skyboxMode", 2);
        if (m_texture_loaded && m_texture && m_texture->IsValid()) {
            // Bind texture using graphics abstraction
            m_texture->Bind(0);
            s_skybox_shader->SetUniform("cubemapTexture", 0);
        }
    }

    s_skybox_shader->SetUniform("textureRotation", m_texture_rotation);
    s_skybox_shader->SetUniform("exposure", m_exposure);

    // Set sun parameters - convert vec4 to vec3 where needed
    s_skybox_shader->SetUniform("sunEnabled", m_sun_enabled);
    s_skybox_shader->SetUniform("sunDirection", GetSunDirection());
    s_skybox_shader->SetUniform("sunColor", glm::vec3(m_sun_color));
    s_skybox_shader->SetUniform("sunIntensity", m_sun_intensity);
...
void Skybox3D::RenderProceduralSky() {
    ...
    // Set uniforms - convert vec4 to vec3 where needed
    s_skybox_shader->SetUniform("skyboxMode", 3);
    s_skybox_shader->SetUniform("sunEnabled", m_sun_enabled);
    s_skybox_shader->SetUniform("sunDirection", GetSunDirection());
    s_skybox_shader->SetUniform("sunColor", glm::vec3(m_sun_color));
    s_skybox_shader->SetUniform("sunIntensity", m_sun_intensity);
