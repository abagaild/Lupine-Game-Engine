...
void Sprite3D::RenderSprite(Node3D* node3d) {
    ...
    if (m_texture_loaded && m_texture_id != 0) {
        Renderer::RenderMesh(temp_mesh, final_transform, m_modulate, m_texture_id, m_receives_lighting);
    } else {
        // Render without texture (white quad with modulate color) - use white texture with lighting control
        Renderer::RenderMesh(temp_mesh, final_transform, m_modulate, 0, m_receives_lighting);
    }
}
