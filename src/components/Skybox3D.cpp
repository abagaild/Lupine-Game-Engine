...
std::unique_ptr<Shader> Skybox3D::s_skybox_shader = nullptr;
std::unique_ptr<Shader> Skybox3D::s_procedural_sky_shader = nullptr;
...
void Skybox3D::SetFogUniforms() {
    ...
    s_skybox_shader->SetVector3("fogColor", m_fog_color);
    ...
    s_skybox_shader->SetVector3("cameraPosition", camera_pos);
}
...
void Skybox3D::InitializeShaders() {
    ...
    s_skybox_shader = std::make_unique<Shader>(skybox_vertex, skybox_fragment);
...
void Skybox3D::RenderSolidColor() {
    ...
    s_skybox_shader->SetVector4("backgroundColor", m_background_color);

    // Set sun parameters (even for solid color mode)
    s_skybox_shader->SetInt("sunEnabled", m_sun_enabled ? 1 : 0);
    s_skybox_shader->SetVector3("sunDirection", GetSunDirection());
    s_skybox_shader->SetVector3("sunColor", m_sun_color);
    s_skybox_shader->SetFloat("sunIntensity", m_sun_intensity);
    s_skybox_shader->SetVector3("shadowColor", m_shadow_color);
    ...
    s_skybox_shader->SetMatrix4("view", view);
    s_skybox_shader->SetMatrix4("projection", Renderer::GetProjectionMatrix());
