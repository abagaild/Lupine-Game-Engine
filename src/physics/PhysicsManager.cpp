...
void PhysicsBody3D::SetCollisionLayer(int layer) {
    if (m_body && m_body->getCollisionShape()) {
        // In Bullet3, collision filtering is done through collision groups and masks
        // Set the collision group (layer) - use bit shifting for layer representation
        short group = static_cast<short>(1 << layer);

        // Update the collision filter
        m_body->getBroadphaseHandle()->m_collisionFilterGroup = group;

        std::cout << "PhysicsBody3D: Set collision layer " << layer
                  << " (group=" << group << ")" << std::endl;
    }
}

void PhysicsBody3D::SetCollisionMask(int mask) {
    if (m_body && m_body->getCollisionShape()) {
        // Set the collision mask (which layers this body can collide with)
        short collision_mask = static_cast<short>(mask);

        // Update the collision filter
        m_body->getBroadphaseHandle()->m_collisionFilterMask = collision_mask;

        std::cout << "PhysicsBody3D: Set collision mask " << mask
                  << " (mask=" << collision_mask << ")" << std::endl;
    }
}

void PhysicsBody3D::SetIsSensor(bool is_sensor) {
    if (m_body && m_body->getCollisionShape()) {
        // In Bullet3, sensors are implemented using collision flags
        if (is_sensor) {
            // Set as sensor - no contact response but still triggers collision events
            m_body->setCollisionFlags(m_body->getCollisionFlags() | btCollisionObject::CF_NO_CONTACT_RESPONSE);
        } else {
            // Remove sensor flag - normal collision response
            m_body->setCollisionFlags(m_body->getCollisionFlags() & ~btCollisionObject::CF_NO_CONTACT_RESPONSE);
        }

        std::cout << "PhysicsBody3D: Set sensor mode " << (is_sensor ? "enabled" : "disabled") << std::endl;
    }
}

bool PhysicsBody3D::IsAwake() const {
    if (m_body) {
...
bool PhysicsBody3D::IsSleeping() const {
    return !IsAwake();
}

glm::vec3 PhysicsBody3D::GetCenterOfMass() const {
    if (m_body) {
        btVector3 com = m_body->getCenterOfMassPosition();
        return glm::vec3(com.x(), com.y(), com.z());
    }
    return glm::vec3(0.0f);
}

float PhysicsBody3D::GetInertia() const {
    if (m_body) {
        btVector3 inertia = m_body->getLocalInertia();
        // Return the magnitude of the inertia tensor
