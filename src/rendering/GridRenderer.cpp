...
std::unique_ptr<Shader> GridRenderer::s_grid_shader;
...
void GridRenderer::Render2DGrid(Camera* camera, const GridConfig& config) {
    ...
    if (!s_initialized || !s_grid_shader) {
    ...
    s_grid_shader->SetMatrix4("model", model);
    s_grid_shader->SetMatrix4("view", camera->GetViewMatrix());
    s_grid_shader->SetMatrix4("projection", camera->GetProjectionMatrix());
...
void GridRenderer::CreateGridShader() {
    ...
    s_grid_shader = std::make_unique<Shader>(vertex_shader, fragment_shader);
}
