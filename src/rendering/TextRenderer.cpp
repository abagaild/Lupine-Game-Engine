...
void TextRenderer::RenderGlyph(const Glyph& glyph, const glm::vec2& position, float scale, const glm::vec4& color) {
    if (s_batching_enabled) {
        ...
        instance.texture = glyph.graphics_texture;
        ...
        // Render glyph using unified graphics system
        Renderer::RenderTextGlyph(transform, color, glyph.graphics_texture);
    }
}
...
void TextRenderer::FlushBatch() {
    ...
    std::unordered_map<std::shared_ptr<GraphicsTexture>, std::vector<GlyphInstance*>> texture_groups;
    for (auto& glyph : s_current_batch.glyphs) {
        texture_groups[glyph.texture].push_back(&glyph);
    }
    ...
    for (const auto& group : texture_groups) {
        ...
        for (const auto& glyph : sorted_glyphs) {
            // Render glyph using unified graphics system
            Renderer::RenderTextGlyph(glyph->transform, glyph->color, glyph->texture);
        }
    }
