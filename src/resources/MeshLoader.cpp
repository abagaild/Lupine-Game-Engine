...
void Mesh::SetupMesh() {
    ...
    if (!graphics_device) {
        std::cerr << "Mesh::SetupMesh: No graphics device available" << std::endl;
        return;
    }

    try {
        // Create vertex array
        VAO = graphics_device->CreateVertexArray();
        if (!VAO) {
            std::cerr << "Mesh::SetupMesh: Failed to create vertex array" << std::endl;
            return;
        }

        // Create vertex buffer
        VBO = graphics_device->CreateBuffer(
            BufferType::Vertex,
            BufferUsage::Static,
        ...
        if (!VBO) {
            std::cerr << "Mesh::SetupMesh: Failed to create vertex buffer" << std::endl;
            return;
        }

        // Create index buffer if we have indices
        if (!indices.empty()) {
            EBO = graphics_device->CreateBuffer(
                BufferType::Index,
                BufferUsage::Static,
            ...
            if (!EBO) {
                std::cerr << "Mesh::SetupMesh: Failed to create index buffer" << std::endl;
                return;
            }
        }

        // Set up vertex layout - must match the Vertex structure exactly
        VertexLayout layout;
        ...
        layout.AddAttribute(3, 3, static_cast<uint32_t>(VertexAttributeType::Float), false); // Tangent
        layout.AddAttribute(4, 3, static_cast<uint32_t>(VertexAttributeType::Float), false); // Bitangent

        // Always add bone attributes since they're always present in the Vertex structure
        // Even if the mesh doesn't use bones, the data is still there (with default values)
        layout.AddAttribute(5, 4, static_cast<uint32_t>(VertexAttributeType::Int), false);   // Bone IDs
        layout.AddAttribute(6, 4, static_cast<uint32_t>(VertexAttributeType::Float), false); // Bone weights
        ...
        std::cout << "Mesh::SetupMesh: Successfully set up mesh with "
...
void Mesh::Draw() const {
    ...
    for (const auto& texture_map : material.diffuse_maps) {
        if (texture_map.texture && texture_unit < 16) { // Limit to 16 texture units
            texture_map.texture->Bind(texture_unit);
            texture_unit++;
        }
    }

    // Draw mesh using graphics device
    VAO->Bind();
    if (!indices.empty()) {
        graphics_device->DrawIndexed(PrimitiveType::Triangles, static_cast<int>(indices.size()), 0);
    } else {
        graphics_device->Draw(PrimitiveType::Triangles, static_cast<int>(vertices.size()), 0);
    }
}
...
void Model::Draw() const {
    for (const auto& mesh : m_meshes) {
        // Ensure mesh is set up for rendering (lazy initialization)
        if (!mesh.VAO) {
...
Mesh Model::ProcessMesh(aiMesh* mesh, const aiScene* scene) {
    ...
    result.has_bones = mesh->HasBones(); // Set bone flag based on source mesh
...
std::vector<MeshTexture> Model::LoadMaterialTextures(aiMaterial* mat, unsigned int type, const std::string& type_name) {
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(static_cast<aiTextureType>(type)); i++) {
        ...
        if (!skip) {
            ...
            mesh_texture.texture = LoadTextureFromFile(texture_path, m_directory);
            mesh_texture.type = type_name;
            mesh_texture.path = texture_path;

            // Only add texture if it was successfully loaded
            if (mesh_texture.texture && mesh_texture.texture->IsValid()) {
                textures.push_back(mesh_texture);
                m_loaded_textures.push_back(mesh_texture);
            } else {
...
std::shared_ptr<GraphicsTexture> Model::LoadTextureFromFile(const std::string& path, const std::string& directory) {
    // Validate input parameters
    if (path.empty()) {
    ...
    std::cout << "Loaded texture: " << filename << " (Handle: " << texture.GetID() << ")" << std::endl;
    return texture.graphics_texture;
}
