...
Texture ResourceManager::LoadTexture(const std::string& path, bool flip_vertically) {
    ...
    texture.id = CreateGLTexture(data, texture.width, texture.height, texture.channels);
...
std::unordered_map<char, Glyph> ResourceManager::GenerateGlyphAtlas(const Font& font) {
    ...
    for (unsigned char c = 32; c < 127; c++) {
        ...
        if (surface->w > 0 && surface->h > 0) {
            ...
            if (rgba_surface) {
                ...
                if (rgba_surface->pixels) {
                    ...
                    glyph.texture_id = CreateHighQualityGlyphTexture(
        ...
        if (glyph.texture_id != 0) {
            glyphs[c] = glyph;
            std::cout << "Successfully created glyph for '" << (char)c << "' with texture ID: " << glyph.texture_id << std::endl;
        } else {
...
void ResourceManager::ClearCache() {
    ...
    for (auto& atlas_pair : s_glyph_cache) {
        for (auto& glyph_pair : atlas_pair.second) {
            if (glyph_pair.second.texture_id != 0) {
...
unsigned int ResourceManager::CreateGLTexture(unsigned char* data, int width, int height, int channels) {
    unsigned int id;
    glGenTextures(1, &id);
    glBindTexture(GL_TEXTURE_2D, id);

    // Fix OpenGL texture upload alignment issues for proper text rendering
    ...
    // Set filtering based on current texture filter setting
    GLenum minFilter, magFilter;
    switch (s_texture_filter) {
        ...
        case TextureFilter::Bicubic:
            // OpenGL doesn't have native bicubic, so we use linear
...
unsigned int ResourceManager::CreateGlyphTexture(unsigned char* data, int width, int height) {
    unsigned int id;
    glGenTextures(1, &id);
    glBindTexture(GL_TEXTURE_2D, id);

    // Fix OpenGL texture upload alignment issues for proper glyph rendering
    // Force 1-byte alignment to handle any padding correctly
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);

    // Zero out any skips to ensure clean upload
    glPixelStorei(GL_UNPACK_SKIP_ROWS, 0);
    glPixelStorei(GL_UNPACK_SKIP_PIXELS, 0);
    glPixelStorei(GL_UNPACK_ROW_LENGTH, 0); // Use width (tightly packed)

    // Set texture parameters optimized for text rendering
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);

    // Use linear filtering for smooth text at different scales
    ...
    // Don't generate mipmaps for small glyph textures
    // glGenerateMipmap(GL_TEXTURE_2D);
...
unsigned int ResourceManager::CreateHighQualityGlyphTexture(unsigned char* data, int width, int height, int pitch) {
    unsigned int id;
    glGenTextures(1, &id);
    glBindTexture(GL_TEXTURE_2D, id);

    // Fix OpenGL texture upload alignment issues for proper high-quality glyph rendering
    // Force 1-byte alignment to handle any padding correctly
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    ...
    // Set row length to handle pitch correctly - pitch is bytes per row for RGBA (4 bytes per pixel)
    // For RGBA format, convert pitch from bytes to pixels
    if (pitch > 0) {
        glPixelStorei(GL_UNPACK_ROW_LENGTH, pitch / 4); // 4 bytes per RGBA pixel
    } else {
        glPixelStorei(GL_UNPACK_ROW_LENGTH, 0); // Use width (tightly packed)
    }

    // Handle row stride properly - pitch is bytes per row, need to convert to pixels for GL_UNPACK_ROW_LENGTH
    if (pitch > 0 && pitch != width * 4) {
        // For RGBA format, row length is in pixels, so divide pitch by 4 (bytes per pixel)
        glPixelStorei(GL_UNPACK_ROW_LENGTH, pitch / 4);
        std::cout << "Setting GL_UNPACK_ROW_LENGTH to " << (pitch / 4) << " pixels (pitch: " << pitch << ", width: " << width << ")" << std::endl;
    } else {
        glPixelStorei(GL_UNPACK_ROW_LENGTH, 0); // Use width (tightly packed)
    }
    ...
    // Use high-quality filtering for crisp text
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    // Use high-quality filtering for crisp text (skip anisotropic for now)
    // Note: Anisotropic filtering can be added later if needed

    // Upload texture data as RGBA with higher precision
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);
    ...
    std::cout << "Created high-quality glyph texture with ID: " << id << " (size: " << width << "x" << height << ")" << std::endl;
    return id;
}
