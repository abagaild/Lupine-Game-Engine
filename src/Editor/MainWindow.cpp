...
{
    FocusedPanel focusedPanel = getCurrentFocusedPanel();

    switch (focusedPanel) {
        case FocusedPanel::SceneTree:
            ...
            break;
        case FocusedPanel::ScriptEditor:
            // Delegate to script editor panel for text operations
            if (m_scriptEditorPanel) {
                // Script editor should handle its own cut operations
                statusBar()->showMessage("Cut operation delegated to script editor");
            }
            break;
        default:
            // Default to scene tree operations for backward compatibility
            if (m_sceneTreePanel && m_currentScene) {
                Lupine::Node* selectedNode = m_sceneTreePanel->getSelectedNode();
                if (selectedNode && selectedNode != m_currentScene->GetRootNode()) {
                    if (m_clipboard) {
...
void MainWindow::onCopy()
{
    ...
    switch (focusedPanel) {
        ...
        case FocusedPanel::ScriptEditor:
            // Delegate to script editor panel for text operations
            if (m_scriptEditorPanel) {
                // Script editor should handle its own copy operations
                statusBar()->showMessage("Copy operation delegated to script editor");
            }
            break;
        default:
            // Default to scene tree operations for backward compatibility
...
void MainWindow::onPaste()
{
    ...
    switch (focusedPanel) {
        ...
        case FocusedPanel::ScriptEditor:
            // Delegate to script editor panel for text operations
            if (m_scriptEditorPanel) {
                // Script editor should handle its own paste operations
                statusBar()->showMessage("Paste operation delegated to script editor");
            }
            break;
        default:
            // Default to scene tree operations for backward compatibility
...
void MainWindow::onDelete()
{
    ...
    switch (focusedPanel) {
        ...
        case FocusedPanel::ScriptEditor:
            // Delegate to script editor panel for text operations
            if (m_scriptEditorPanel) {
                // Script editor should handle its own delete operations
                statusBar()->showMessage("Delete operation delegated to script editor");
            }
            break;
        default:
            // Default to scene tree operations for backward compatibility
...
void MainWindow::onDuplicate()
{
    ...
    switch (focusedPanel) {
        ...
        case FocusedPanel::ScriptEditor:
            // Delegate to script editor panel for text operations
            if (m_scriptEditorPanel) {
                // Script editor should handle its own duplicate operations
                statusBar()->showMessage("Duplicate operation delegated to script editor");
            }
            break;
        default:
            // Default to scene tree operations for backward compatibility
...
void MainWindow::initializeEngine()
{
    ...
    // NOTE: Graphics device and renderer initialization is deferred until SceneViewPanel::initializeGL()
    // because we need a valid OpenGL context first

    // Initialize non-graphics systems first
...
MainWindow::FocusedPanel MainWindow::getCurrentFocusedPanel() const
{
    ...
    // Check if the focused widget is within any of our panels
    // We need to traverse up the widget hierarchy to find the panel
    QWidget* current = focusedWidget;
    while (current) {
        // Check if current widget is one of our panels
        if (current == m_sceneTreePanel) {
            return FocusedPanel::SceneTree;
        }
        if (current == m_scriptEditorPanel) {
            return FocusedPanel::ScriptEditor;
        }
        if (current == m_assetBrowserPanel) {
            return FocusedPanel::AssetBrowser;
        }
        if (current == m_inspectorPanel) {
            return FocusedPanel::Inspector;
        }
        if (current == m_consolePanel) {
            return FocusedPanel::Console;
        }
        if (current == m_sceneViewPanel) {
            return FocusedPanel::SceneView;
        }

        // Check if current widget is within any of our dock widgets
        if (current == m_sceneTreeDock->widget()) {
            return FocusedPanel::SceneTree;
        }
        if (current == m_scriptEditorDock->widget()) {
            return FocusedPanel::ScriptEditor;
        }
        if (current == m_assetBrowserDock->widget()) {
            return FocusedPanel::AssetBrowser;
        }
        if (current == m_inspectorDock->widget()) {
            return FocusedPanel::Inspector;
        }
        if (current == m_consoleDock->widget()) {
            return FocusedPanel::Console;
        }
