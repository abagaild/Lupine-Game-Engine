...
void ExportDialog::setupUI() {
    ...
    m_errorLabel->setStyleSheet("QLabel { color: red; background-color: #ffe6e6; padding: 8px; border: 1px solid #ff9999; border-radius: 4px; }");
    ...
    connect(m_browseMacIconButton, &QPushButton::clicked,
            this, &ExportDialog::onBrowseMacIcon);
...
void ExportDialog::setupPlatformSpecificTab() {
    ...
    // Mac settings
    m_macGroup = new QGroupBox("Mac Settings");
    auto* macLayout = new QGridLayout(m_macGroup);

    row = 0;
    macLayout->addWidget(new QLabel("Bundle Identifier:"), row, 0);
    m_bundleIdentifierEdit = new QLineEdit();
    m_bundleIdentifierEdit->setPlaceholderText("com.company.appname");
    macLayout->addWidget(m_bundleIdentifierEdit, row++, 1);

    macLayout->addWidget(new QLabel("Icon:"), row, 0);
    auto* macIconLayout = new QHBoxLayout();
    m_macIconEdit = new QLineEdit();
    m_browseMacIconButton = new QPushButton("Browse...");
    macIconLayout->addWidget(m_macIconEdit);
    macIconLayout->addWidget(m_browseMacIconButton);
    macLayout->addLayout(macIconLayout, row++, 1);

    macLayout->addWidget(new QLabel("Version:"), row, 0);
    m_macVersionEdit = new QLineEdit("1.0.0");
    macLayout->addWidget(m_macVersionEdit, row++, 1);

    macLayout->addWidget(new QLabel("Minimum OS Version:"), row, 0);
    m_minimumOSEdit = new QLineEdit("10.15");
    macLayout->addWidget(m_minimumOSEdit, row++, 1);

    m_createDMGCheck = new QCheckBox("Create DMG");
    m_createDMGCheck->setChecked(true);
    macLayout->addWidget(m_createDMGCheck, row++, 1);

    m_codeSignCheck = new QCheckBox("Code Sign");
    macLayout->addWidget(m_codeSignCheck, row++, 1);

    macLayout->addWidget(new QLabel("Developer ID:"), row, 0);
    m_developerIdEdit = new QLineEdit();
    m_developerIdEdit->setPlaceholderText("Developer ID Application: ...");
    macLayout->addWidget(m_developerIdEdit, row++, 1);

    layout->addWidget(m_macGroup);
...
void ExportDialog::populateTargetComboBox() {
    auto exporters = m_export_manager->GetAllExporters();

    for (auto* exporter : exporters) {
        QString name = QString::fromStdString(exporter->GetName());
        if (!exporter->IsAvailable()) {
            name += " [Unavailable]";
        }
        m_targetComboBox->addItem(name, static_cast<int>(exporter->GetTarget()));
    }
...
void ExportDialog::updatePlatformSpecificSettings() {
    ...
    m_macGroup->setVisible(false);
    m_webGroup->setVisible(false);

    // Get the exporter for this target to check availability
    BaseExporter* exporter = m_export_manager->GetExporter(target);
    if (!exporter) {
        // Find the exporter in all exporters list
        auto all_exporters = m_export_manager->GetAllExporters();
        for (auto* exp : all_exporters) {
            if (exp->GetTarget() == target) {
                exporter = exp;
                break;
            }
        }
    ...
    // Show error message if exporter is not available
    if (exporter && !exporter->IsAvailable()) {
        std::string error = exporter->GetAvailabilityError();
    ...
    switch (target) {
        ...
        case ExportTarget::Mac_x64:
            m_macGroup->setVisible(true);
            if (m_executableNameEdit->text().isEmpty()) {
                m_executableNameEdit->setText("Game.app");
            }
            if (m_bundleIdentifierEdit->text().isEmpty()) {
                m_bundleIdentifierEdit->setText(QString("com.lupine.%1").arg(QString::fromStdString(m_project->GetName()).toLower()));
            }
            break;
...
void ExportDialog::onBrowseMacIcon() {
    QString file = QFileDialog::getOpenFileName(this, "Select Mac Icon File",
                                               QString(),
                                               "Icon Files (*.icns *.png *.jpg *.bmp)");
    if (!file.isEmpty()) {
        m_macIconEdit->setText(file);
    }
}
...
ExportConfig ExportDialog::getConfigFromUI() const {
    ...
    config.mac.bundle_identifier = m_bundleIdentifierEdit->text().toStdString();
    config.mac.icon_path = m_macIconEdit->text().toStdString();
    config.mac.version_info = m_macVersionEdit->text().toStdString();
    config.mac.minimum_os_version = m_minimumOSEdit->text().toStdString();
    config.mac.create_dmg = m_createDMGCheck->isChecked();
    config.mac.code_sign = m_codeSignCheck->isChecked();
    config.mac.developer_id = m_developerIdEdit->text().toStdString();
...
void ExportDialog::setConfigToUI(const ExportConfig& config) {
    ...
    m_bundleIdentifierEdit->setText(QString::fromStdString(config.mac.bundle_identifier));
    m_macIconEdit->setText(QString::fromStdString(config.mac.icon_path));
    m_macVersionEdit->setText(QString::fromStdString(config.mac.version_info));
    m_minimumOSEdit->setText(QString::fromStdString(config.mac.minimum_os_version));
    m_createDMGCheck->setChecked(config.mac.create_dmg);
    m_codeSignCheck->setChecked(config.mac.code_sign);
    m_developerIdEdit->setText(QString::fromStdString(config.mac.developer_id));
...
void ExportDialog::validateAndEnableExport() {
    ...
    if (valid && !m_targetComboBox->currentData().isNull()) {
        ExportTarget target = static_cast<ExportTarget>(m_targetComboBox->currentData().toInt());
        BaseExporter* exporter = m_export_manager->GetExporter(target);
        if (!exporter) {
            valid = false;
