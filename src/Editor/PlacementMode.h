...
private:
    ...
    // Visual indicator rendering helpers
    void renderGridIndicators(Lupine::Camera* camera);
    void renderSurfaceNormalIndicators(Lupine::Camera* camera, const glm::mat4& viewProjection);
    void renderSnapGuides(Lupine::Camera* camera, const glm::mat4& viewProjection);
    void renderPlacementBounds(Lupine::Camera* camera, const glm::mat4& viewProjection);
    void renderNearbyObjectIndicators(Lupine::Camera* camera, const glm::mat4& viewProjection);
    ...
    std::shared_ptr<Lupine::GraphicsTexture> m_ghostTexture;
    bool m_ghostTextureLoaded = false;
    glm::vec2 m_ghostSpriteSize{1.0f};
    glm::vec3 m_lastSurfaceNormal{0.0f, 1.0f, 0.0f}; // Store last detected surface normal
};
