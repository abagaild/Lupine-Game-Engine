...
bool LinuxExporter::IsAvailable() const {
    ...
    // For now, always allow Linux export (cross-compilation will be handled during export)
    // This allows the export option to appear in the UI even without pre-built Linux libraries
    return true;
}

std::string LinuxExporter::GetAvailabilityError() const {
    if (IsAvailable()) {
        return "";
    }
    ...
    if (!embedded_manager.HasLibrariesForPlatform("linux", "x64")) {
        std::filesystem::path linux_libs = "thirdparty/linux_x64-static";
        if (!std::filesystem::exists(linux_libs)) {
            return "Linux static libraries not found. Missing embedded libraries and thirdparty/linux_x64-static directory";
        }
    }

    return "Linux export not available";
}
