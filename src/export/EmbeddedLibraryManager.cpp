...
#ifdef LUPINE_EMBED_MAC_LIBRARIES
#include "embedded_mac_libraries.h"
#endif
...
#ifdef LUPINE_EMBED_MAC_LIBRARIES
    // Load Mac x64 libraries
    const auto& mac_libs = GetEmbeddedMacLibraries();

    std::vector<std::string> mac_lib_names;

    for (const auto& [name, data] : mac_libs) {
        auto lib = std::make_unique<EmbeddedLibrary>();
        lib->name = name;
        lib->platform = "mac";
        lib->architecture = "x64";
        ...
        std::string key = CreateLibraryKey(name, "mac", "x64");
        m_libraries[key] = std::move(lib);

        mac_lib_names.push_back(name);
    }

    m_platform_libraries["mac_x64"] = mac_lib_names;

    std::cout << "Loaded " << mac_libs.size() << " embedded Mac libraries" << std::endl;
    loaded_any = true;
#endif
