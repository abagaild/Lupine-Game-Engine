...
bool WindowsExporter::IsAvailable() const {
    ...
    bool runtime_exists = std::filesystem::exists(runtime_path);

    // For debugging: print the path being checked (only in debug builds)
    #ifdef _DEBUG
    if (!runtime_exists) {
        std::cerr << "Windows exporter: Runtime not found at " << runtime_path << std::endl;

        // List what files do exist in the build directory
        try {
            std::filesystem::path build_dir = "build/bin/Release";
            if (std::filesystem::exists(build_dir)) {
                std::cerr << "Files in " << build_dir << ":" << std::endl;
            ...
            std::cerr << "Error listing build directory" << std::endl;
        }
    }
    #endif
...
std::string WindowsExporter::GetAvailabilityError() const {
    if (IsAvailable()) {
        return "";
    }
...
#ifdef _WIN32
    auto runtime_path = GetRuntimeExecutablePath();
    return "Windows runtime executable not found at: " + runtime_path.string() +
           ". Build the project first with 'cmake --build build --config Release'";
#else
    auto& embedded_manager = GetEmbeddedLibraryManager();
    if (!embedded_manager.HasLibrariesForPlatform("windows", "x64")) {
        std::filesystem::path windows_libs = "thirdparty/windows_x64-static";
        if (!std::filesystem::exists(windows_libs)) {
            return "Windows static libraries not found. Missing embedded libraries and thirdparty/windows_x64-static directory";
        }
        return "Windows runtime executable not found in thirdparty/windows_x64-static";
    }
    return "Windows export not available on this platform";
#endif
}
...
std::filesystem::path WindowsExporter::GetRuntimeExecutablePath() const {
    ...
    try {
        ...
        // Try relative to the editor executable location
        auto exe_path = std::filesystem::current_path() / "build" / "bin" / "Release" / "lupine-runtime.exe";
        if (std::filesystem::exists(exe_path)) {
            return std::filesystem::absolute(exe_path);
        }

        // Try installed location (when engine is distributed)
        auto installed_path = current_dir / "bin" / "lupine-runtime.exe";
        if (std::filesystem::exists(installed_path)) {
            return std::filesystem::absolute(installed_path);
        }
    } catch (...) {
        // Ignore errors
    }
