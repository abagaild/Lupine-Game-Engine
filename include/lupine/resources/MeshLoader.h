...
struct Mesh {
    ...
    // Graphics abstraction objects
    std::shared_ptr<GraphicsVertexArray> VAO;
    std::shared_ptr<GraphicsBuffer> VBO;
    std::shared_ptr<GraphicsBuffer> EBO;

    /**
     * @brief Constructor
     */
    Mesh() : has_bones(false), VAO(nullptr), VBO(nullptr), EBO(nullptr) {}
...
private:
     ...
     */
    std::shared_ptr<GraphicsTexture> LoadTextureFromFile(const std::string& path, const std::string& directory);
