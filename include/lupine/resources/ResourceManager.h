 ...
 */
struct Texture {
    std::shared_ptr<GraphicsTexture> graphics_texture = nullptr;
    int width = 0;
    int height = 0;
    int channels = 0;
    std::string path;
    ...
    // Legacy compatibility - returns native handle for backward compatibility
    unsigned int GetID() const;

    // Legacy compatibility property
    unsigned int id() const { return GetID(); }
};
...
public:
     ...
     * @param graphics_device Graphics device for resource creation
     * @return True if successful
     */
    static bool Initialize(std::shared_ptr<GraphicsDevice> graphics_device = nullptr);
...
private:
    ...
    /**
     * @brief Create graphics texture from image data
     ...
     */
    static std::shared_ptr<GraphicsTexture> CreateTexture(unsigned char* data, int width, int height, int channels);

    /**
     * @brief Create graphics texture optimized for glyph rendering
     * @param data RGBA pixel data
     * @param width Texture width
     * @param height Texture height
     * @return Graphics texture
     */
    static std::shared_ptr<GraphicsTexture> CreateGlyphTexture(unsigned char* data, int width, int height);

    /**
     * @brief Create high-quality graphics texture for glyph rendering with pitch support
     * @param data RGBA pixel data
     * @param width Texture width
     * @param height Texture height
     ...
     * @return Graphics texture
     */
    static std::shared_ptr<GraphicsTexture> CreateHighQualityGlyphTexture(unsigned char* data, int width, int height, int pitch);
